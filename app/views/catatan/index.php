...
<div class="app-card p-4 sm:p-6 max-w-4xl mx-auto">
    <div class="mb-6">
        ...
        <p class="mt-2 text-sm text-slate-500"><?= $isAdmin ? 'Catatan kegiatan dan kendala operasional dari kasir.' : 'Catat kegiatan atau kendala operasional agar dapat dilihat admin.' ?></p>
    </div>
    <?php if (!$isAdmin): ?>
        <form action="/catatan-operasional/store" method="POST" class="rounded-2xl border border-teal-200 bg-teal-50/50 p-4 mb-6">
            ...
            <div class="mt-4">
                <label for="uang_di_kasir" class="block text-sm font-semibold text-slate-700 mb-2">Uang di Kasir *</label>
                <div class="flex items-center rounded-xl border border-slate-300 bg-white overflow-hidden focus-within:ring-2 focus-within:ring-teal-500">
                    <span class="px-3 text-slate-500" aria-hidden="true">Rp</span>
            ...
            <div class="mt-3 flex flex-wrap gap-3 items-center justify-between">
                <span class="text-xs text-slate-500">Maksimal 4.000 karakter.</span>
        ...
        <?php foreach ($notes as $note): ?>
            <article class="rounded-xl border border-slate-200 bg-white p-4">
                <div class="flex flex-wrap justify-between gap-2 mb-3 text-sm">
                    ...
                    <time class="text-xs text-slate-500" datetime="<?= htmlspecialchars($note['created_at'], ENT_QUOTES, 'UTF-8') ?>"><?= formatTanggalWaktu($note['created_at']) ?></time>
                </div>
                ...
                <div class="mb-3 rounded-lg border border-teal-200 bg-teal-50 px-3 py-2">
                    <div class="flex flex-wrap justify-between gap-2">
                        <span class="text-sm text-teal-800">Uang di Kasir</span>
